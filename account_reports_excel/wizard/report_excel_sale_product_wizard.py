...
class ReportExcelSaleProductWizard(models.TransientModel):
    ...
    def get_values(self, date_from, date_to):
        ...
        if self.product_id:
            self._cr.execute(''' select 
                                    ...
                                    CASE WHEN l.discount>0
                                    THEN 0.0
                                    ELSE (l.price_total - l.price_unit)
                                    END AS tax,
                                    CASE WHEN m.type = 'out_refund'
    ...
    def make_file(self, value):
        ...
        ws.write(3, 1, str(self.date_from), title_head)        
        ws.write(4, 0, 'Fecha Inicio', title_head)
        ws.write(4, 1, str(self.date_to), title_head)
        ws.write(4, 2, 'Usuario', title_head)
        ws.write(4, 3, str(user.partner_id.name), title_head)
        ws.write(5, 0, str(date_file), title_head)

        ws.write(6, 0, 'Factura', subtitle_head)
        ws.write(6, 1, 'Cliente', subtitle_head)
        ws.write(6, 2, 'NIT', subtitle_head)
        ws.write(6, 3, 'Fecha de la factura', subtitle_head)
        ws.write(6, 4, 'Fecha de Vencimiento', subtitle_head)
        ws.write(6, 5, 'Documento Origen', subtitle_head)
        ws.write(6, 6, 'Producto', subtitle_head)
        ws.write(6, 7, 'Company', subtitle_head)
        ws.write(6, 8, 'Red de Valor', subtitle_head)        
        ws.write(6, 9, 'Cuenta Analitica', subtitle_head)          
        ws.write(6, 10, 'Estado', subtitle_head)
        ws.write(6, 11, 'Vendedor', subtitle_head)
        ws.write(6, 12, 'Equipo de Venta', subtitle_head)
        ws.write(6, 13, 'Enviado a la DIAN', subtitle_head)
        ws.write(6, 14, 'Fecha de Envio a la DIAN', subtitle_head)        
        ws.write(6, 15, 'CUFE', subtitle_head)        
        ws.write(6, 16, 'Cantidad', subtitle_head)        
        ws.write(6, 17, 'SubTotal', subtitle_head)        
        ws.write(6, 18, 'Descuento', subtitle_head)
        ws.write(6, 19, '% Descuento', subtitle_head)
        ws.write(6, 20, 'Impuesto', subtitle_head)        
        ws.write(6, 21, 'Total', subtitle_head)
        ...
        try:
            ...
            date_file = fields.Datetime.now()
            self.data = out
            self.data_name = 'Facturaporproducto' + '-' + str(date_file)
        except ValueError:
