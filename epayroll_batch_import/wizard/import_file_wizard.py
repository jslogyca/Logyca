...
class EpayrollImportFileWizard(models.TransientModel):
    ...
    def import_file(self):
        ...
        #listas para capturar empleados sin usuario, o empleados no encontrados
        non_user_employees, non_partner_employees, non_employee_employees = ([], ) * 3
        ...
        for fila in record_list:
            ...
            if not partner:
                ...
                # con el partner encontrado puedo buscarlo en usuarios
                user = self.env['res.users'].search([('partner_id','=', partner.id)])
                if not user:
                    non_user_employees.append(employee_name)
                else:
                    ...
                    #paso 4 - buscar la estctructura a la que corresponde el tipo de estructura
        ...
        logging.info("==> payslip_id")
        logging.info(str(payslip_id))
        ...
        logging.info("==> non_user_employees")
        logging.info(str(non_user_employees))
